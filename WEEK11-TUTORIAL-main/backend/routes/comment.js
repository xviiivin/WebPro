...
router.post('/:blogId/comments', upload.single('comment_image'), async function(req, res, next) {
    ...
    try {
        ...
        const [rows, fields] = await pool.query(
            'INSERT INTO comments(comment, blog_id) VALUES (?, ?)', [comment.comment, blog_id]
        )
        console.log("save comment");
        if (file) {
            const [rows2, fields2] = await pool.query('INSERT INTO images(blog_id, comment_id, file_path, upload_date) VALUES(?, ?, ?, CURRENT_TIMESTAMP);', [blog_id, rows.insertId, file.path.substr(6)])
